...
export default function OurOffers() {
    const services = [
  {
    ...
    img: code,
    description: "We build responsive, high-performance websites and dynamic web applications tailored to meet your specific business goals.",
  ...
  },
  {
    title: "UI/UX Design",
    img: uiux,
    description: "Crafting intuitive interfaces (UI) and seamless experiences (UX) to ensure digital products are aesthetically pleasing and easy to use.",
    features: ["Responsive Design", "SEO Optimized", "Fast Loading", "Cross Browser"],
    techs: ["React.js", "Vue.js", "Node.js", "PHP/Laravel"]
  }
];
       ...
       backgroundImage: `url(${grids})`,
         ...
         <div className="relative w-full max-w-[1400px]  overflow-hidden bg-white/[98%] p-10 md:p-20">
          ...
          {services.map((service, index) => (
            <div key={index} className="bg-white rounded-[20px] overflow-hidden shadow-lg border border-gray-100 flex flex-col w-[473px] h-[635px]">
              ...
              <div className="bg-[#374151] p-8 flex flex-col items-center text-center text-[#F1F3F4]">
                ...
                <p className="text-gray-300 text-sm mt-2 font-light">We Create custom websites and web applications</p>
              </div>
              ...
              <div className="p-8 space-y-6 flex-grow">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {service.features.map((feature, idx) => (
                    <div key={idx} className="flex text-[#4C4480CC]/80 items-center gap-2 border border-[#4C448099]/60 rounded-[10px] py-2 px-3">
                      ...
                      <span className="text-[14px] text-[#4C4480CC]/80 font-medium">{feature}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-2">
                    <p className="text-[14px] font-semibold text-[#4C4480CC]/80">Technology:</p>
                    <div className="flex flex-wrap gap-2">
                    {service.techs.map((tech, idx) => (
                        <span key={idx} className={`text-[10px] text-[#F1F3F4] px-3 py-2 rounded-[18px] font-bold ${
                        idx === 0 ? 'bg-[#4ADE80]' : idx === 1 ? 'bg-[#22D3EE]' : idx === 2 ? 'bg-[#A855F7]' : 'bg-[#E11D48]'
                        }`}>
                        {tech}
                        </span>
                    ))}
