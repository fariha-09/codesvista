...
import { LuSquareCode } from "lucide-react"; // Switched to lucide for consistency if needed, or keep your icons
import { MdOutlineLaunch } from "react-icons/md";
import { motion } from 'framer-motion';

export default function OurMarketingProcess() {
  // This state helps track which card is "active" on mobile specifically
  ...
  return (
    <section className="bg-[#2B2C34] md:py-20 py-10 md:px-6 px-2 overflow-hidden">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <motion.div
                key={index}
                initial="initial"
                // On mobile, if this card is clicked/tapped, stay in 'hover' state
                animate={activeCard === index ? "hover" : "initial"} 
                whileHover="hover" // Desktop
                whileTap="hover"   // Mobile: triggers while being touched
                onClick={() => {
                    // This toggle allows users to tap once to see the effect 
                    // and tap again (or elsewhere) to hide it
                    setActiveCard(activeCard === index ? null : index);
                }}
                className="relative cursor-pointer md:p-8 py-4 px-2 bg-white/10 md:rounded-[40px] rounded-xl flex flex-col items-center text-center overflow-hidden border border-white/5"
              >
                {/* Background Animation Layer */}
                <motion.div
                  variants={{
                    initial: { scale: 0, opacity: 0, backgroundColor: "#F1F1F1" },
                    hover: { scale: 1.5, opacity: 1, backgroundColor: "#F1F1F1" },
                  }}
                ...
                <div className="relative z-10 flex flex-col items-center w-full">
                  ...
                  <motion.p 
                    variants={{
                      ...
                      hover: { color: "#444" } // Darker color for readability on white bg
                    }}
                    className="text-[13px] md:text-[14px] leading-relaxed font-medium px-2 transition-colors duration-300"
                  >
                    {step.desc}
                  </motion.p>
                </div>
                ...
                <div className="absolute inset-0 border border-white/10 md:rounded-[40px] rounded-xl pointer-events-none" />
              </motion.div>
            );
          })}
        </div>
