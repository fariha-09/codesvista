      ...
      <div className="relative w-full overflow-hidden bg-white/[98%] px-4 py-16 md:p-20">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-8">
          {stats.map((item, index) => (
            ...
            >
              {/* Icon Container */}
              <div className="w-16 h-16 bg-white text-[#E53E3E] rounded-2xl shadow-[0_8px_25px_rgba(0,0,0,0.06)] flex items-center justify-center md:mb-6 mb-2 transition-transform group-hover:scale-110">
                {item.icon}
              </div>

              {/* Data Value with Counter Hooked In */}
              <h3 className="text-[#AC1B9E] text-3xl md:text-4xl font-extrabold mb-2">
                <Counter value={item.value} suffix={item.suffix} />
              </h3>

              {/* Label */}
              <p className="text-[#AC1B9E] font-medium text-[10px] md:text-sm uppercase tracking-wider px-2">
                {item.label}
              </p>
            </motion.div>
          ))}
