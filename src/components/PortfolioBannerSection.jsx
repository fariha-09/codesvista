...
export default function PortfolioBannerSection() {
  ...
  return (
    <section className="relative bg-[#0fc8ca] min-h-[88vh] w-full flex justify-center font-sans overflow-hidden">
      <div className="relative w-full max-w-[1500px] h-full flex flex-col justify-center">
        {/* Background Grid */}
        ...
        <div className="container mx-auto px-6 md:px-10 py-10 lg:py-4 relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10">
          ...
          <div className="w-full lg:w-1/2 space-y-6 md:space-y-8 text-left">
            ...
            <p className="text-[#F1F3F4] text-base md:text-lg font-normal max-w-xl leading-relaxed">
              Discover our portfolio of successful projects that demonstrate our
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                { label: "Quick Response", icon: FaLightbulb, iconColor: "#4ADE80" },
              ].map((tag, idx) => (
                <motion.span
                  key={idx}
                  initial="initial"
                  whileHover="hover"
                  className="relative flex items-center gap-2 bg-white px-3 py-2 md:px-4 md:py-3 rounded-lg text-[11px] md:text-[12px] font-bold text-[#4C4480] cursor-pointer overflow-hidden shadow-sm"
                >
                  <motion.div
                    variants={{ initial: { scaleX: 0 }, hover: { scaleX: 1 } }}
                    transition={{ duration: 0.4 }}
                    className="absolute inset-0 bg-gray-100 z-0 origin-center"
                  />
                  <tag.icon size={14} className="relative z-10" style={{ color: tag.iconColor }} />
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <button className="flex items-center justify-center gap-2 bg-[#0FC8CA] hover:bg-white hover:text-[#0FC8CA] text-[14px] text-[#2B2C34] px-8 py-4 rounded-xl font-bold shadow-lg transition-all active:scale-95 w-full sm:w-auto">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="overflow-hidden">
                        <h4 className="text-white font-bold text-lg md:text-xl leading-tight group-hover:text-[#0FC8CA] transition-colors truncate">
                          {stat.label}
                        </h4>
                        <p className="text-white/70 text-xs md:text-sm group-hover:text-[#0FC8CA] transition-colors truncate">
                          {stat.sub}
                        </p>
                      </div>
                    </div>
